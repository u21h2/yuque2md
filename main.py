...
def handler(origin_md_path, output_md_path, image_dir, image_url_prefix, image_rename_mode):
    ...
    with open(origin_md_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f.readlines():
            ...
            image_url = str(re.findall(r'\[image.png\]\(http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+',line))
            if yuque_cdn_domain in image_url and '.png' in image_url:
                image_url = image_url.replace('[image.png]', '').replace('(', '').replace(')', '').replace('[', '').replace(']', '').replace("'", '')
                download_image(image_url, image_dir, image_rename_mode, idx)
                to_replace = '/'.join(image_url.split('/')[:-1])
                new_image_url = image_url.replace(to_replace, 'placeholder')
                if image_rename_mode == 'asc':
                    new_image_url = image_url_prefix + image_file_prefix + str(idx) + '.png'
                else:
...
def download_image(image_url, image_dir, image_name_mode, idx):
    r = requests.get(image_url, stream=True)
    image_name = image_url.split('/')[-1]
    if image_name_mode == 'asc':
        image_name = image_file_prefix + str(idx) + '.png'
